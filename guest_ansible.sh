...
then
  ...
  vagrant ssh -- 'cd /vagrant/suma/analysis && /usr/bin/bower install'
elif [ "$1" == "bower-clean" ]
then
...
then
  ...
  vagrant ssh -- 'cd /vagrant/ansible && /usr/bin/ansible-playbook /vagrant/ansible/demo.yml --tags=npm-install --inventory=/vagrant/ansible/inventories/development.ini'
elif [ "$1" == "npm-clean" ]
then
...
then
  ...
  vagrant ssh -- 'source /home/vagrant/.bash_profile; cd /vagrant/suma/analysis && /usr/bin/grunt'
elif [ "$1" == "grunt-test" ]
then
...
then
  ...
  vagrant ssh -- 'source /home/vagrant/.bash_profile; cd /vagrant/suma/analysis && /usr/bin/grunt build'
elif [ "$1" == "grunt-watch" ]
then
  echo "Running grunt watch on guest..."
  echo "NOTE: this task will continue to run"
  vagrant ssh -- 'source /home/vagrant/.bash_profile; cd /vagrant/suma/analysis && /usr/bin/grunt watch'
elif [ "$1" == "grunt-lint" ]
then
  echo "Running grunt lint on guest..."
  vagrant ssh -- 'cd /vagrant/suma/analysis && /usr/bin/grunt lint'
else
